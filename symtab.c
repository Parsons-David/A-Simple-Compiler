...
void
insert(char *name, Type_Expression type, int offset, varType var_type){
  int currentIndex;
  int visitedSlots = 0;
  ...
  HashTable[currentIndex]->var_type = var_type; /* varType */
}
...
PrintSymbolTable() {
  ...
  for (i=0; i < HASH_TABLE_SIZE; i++) {
    if (HashTable[i] != NULL) {
      char * var_type = (HashTable[i]->var_type == TYPE_ARRAY ? "ARRAY" : "SCALAR");
